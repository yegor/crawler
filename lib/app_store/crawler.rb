...
module AppStore  
  class Crawler
    class << self
      ...
      def sync
        ...
        Chart.find_in_batches(:batch_size => 10) do |charts|
          charts.each do |chart|
            ...
            AppStore::Crawler.fetch(:url => chart.url).each_with_index do |entry_attributes, index|
              ...
              GameSnapshot.create :game => game, :meta_data => meta_data, :rank => (index + 1)
            end
      ...
      #
      def fetch(feed_url)
        JSON.parse(open(feed_url).read)["feed"]["entry"].first
      end
    end
  end
end